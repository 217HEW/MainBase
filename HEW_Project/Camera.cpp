...
//*****************************************************************************
namespace {
	const float CAM_POS_P_X = -150.0f;					// カメラの視点初期位置(X座標)
	const float CAM_POS_P_Y = -250.0f;				// カメラの視点初期位置(Y座標)
	const float CAM_POS_P_Z = -250.0f;				// カメラの視点初期位置(Z座標)
	const float CAM_POS_R_X = 0.0f;					// カメラの注視点初期位置(X座標)
	const float CAM_POS_R_Y = -250.0f;				// カメラの注視点初期位置(Y座標)
...
void CCamera::Update()
{
	...
	// 追跡カメラ&
	XMFLOAT3 vModelPos = XMFLOAT3(50.0f, 0.0f, 0.0f);// GetPlayerPos();	// 自機座標
