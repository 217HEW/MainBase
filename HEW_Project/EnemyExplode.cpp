...
//#define EXPLODE_RANGE			(30.0f)	//爆発的のサイズの外側の爆風範囲
//#define BLOCK_SIZE				(40.0f)//当たり判定用のサイズ
//
//
////**************************************************************
//// グローバル変数
...
//				if ((abs(g_EExplode[i].m_pos.x - posPlayer.x) <= EXPLODE_RANGE) && (abs(g_EExplode[i].m_pos.y - posPlayer.y) <= EXPLODE_RANGE))
//				{
...
//
//					//爆発範囲
//					//if (CollisionSphere(g_EExplode[i].m_pos,g_EExplode[i].m_size.x,Block->m_pos, BLOCK_SIZE))
//					//{
//					//	//敵のx座標がプレイヤーよりも大きかったら
//					//	if (EXPLODE_RANGE >= BLOCK_SIZE)
//					//	{
//					//		
//					//	}
//					//	//敵のx座標がプレイヤーよりも小さかったら
//					//	if (g_EExplode[i].m_size.x - EXPLODE_RANGE <= BLOCK_SIZE)
//					//	{
//					//		
//					//
//					//	}
//					//	//敵のy座標がプレイヤーよりも大きかったら
//					//	if (g_EExplode[i].m_size.y + EXPLODE_RANGE >= BLOCK_SIZE)
//					//	{
//					//		
//					//
//					//	}
...
//
//					
//						//絶対値を用いたブロックと爆発範囲の当たり判定
//						//if ((abs(g_EExplode[i].m_pos.x - Block->m_pos.x) <= EXPLODE_RANGE) && (abs(g_EExplode[i].m_pos.y - Block->m_pos.y) <= EXPLODE_RANGE))
//						//{
//						//
//						//	//タイマーの値が、0よりも小さくなったら
//						//	//ライフを削り、エネミーを削除する
